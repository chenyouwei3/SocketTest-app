...
func LoginUser(user model.User) utils.Response {
	...
	if err != nil {
		return utils.ErrorMess("生成token失败", err)
	}
...
func RegisterUser(user model.User) utils.Response {
	//查询
	fmt.Println(user)
	if user.Account == "" || user.Password == "" {
...
func DeletedUser(id int) utils.Response {
...
func GetUser(currPage, pageSize, startTime, endTime string) utils.Response {
